...
class SamCell(nn.Module):
    def __init__(self, in_channel, num_hidden, width, kernel_size, stride, layer_norm):
        ...
        if layer_norm:
            self.W_C = nn.Sequential(
                nn.Conv2d(in_channel + num_hidden, num_hidden * 3,
                          kernel_size=kernel_size, stride=stride, padding=self.padding, bias=False),
                nn.LayerNorm([num_hidden * 3, width, width])
            )
            self.W_O = nn.Sequential(
                nn.Conv2d(in_channel + num_hidden, num_hidden,
                          kernel_size=kernel_size, stride=stride, padding=self.padding, bias=False),
                nn.LayerNorm([num_hidden * 3, width, width])
            )
        else:
            self.W_C = nn.Conv2d(in_channel + num_hidden, num_hidden * 3,
                                 kernel_size=kernel_size, stride=stride, padding=self.padding, bias=False)
            self.W_O = nn.Conv2d(in_channel + num_hidden, num_hidden,
                                 kernel_size=kernel_size, stride=stride, padding=self.padding, bias=False)
    ...
    def forward(self, x_t: torch.Tensor, h_t: torch.Tensor,
                c_t: torch.Tensor, m_t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
