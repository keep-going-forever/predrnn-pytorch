...
class OriginSelfAttentionMemory(nn.Module):
    def __init__(self, in_channel, num_hidden, width, kernel_size, stride, layer_norm):
        super(OriginSelfAttentionMemory, self).__init__()
    ...
    def forward(self, ns: torch.Tensor) -> torch.Tensor:
        """
        输入:
        ...
        q = self.query(ns).view(batch_size, self.num_hidden, H * W).transpose(1, 2)
        k = self.key(ns).view(batch_size, self.num_hidden, H * W)
        v = self.value(ns).view(batch_size, self.num_hidden, H * W)
